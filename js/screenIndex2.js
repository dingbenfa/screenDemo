    ...
    var app = {
        ...
        getMapChartData: function() { //获取地图图表数据 ----大屏区位要素模块
            ...
            if (isMock) {
                ...
                httpRequest('get', locationFactorsApi, 'enterprise=' + this.enterpriseId, true, 1, 0, null, function(data) {
        ...
        getLineChartData: function() { //获取第二区域图表数据 ----生产状况
            ...
            if (isMock) {
                ...
                httpRequest('get', productionStatusApi, 'enterprise=' + this.enterpriseId, true, 1, 0, null, function(data) {
        ...
        getBarLChartData: function() { //获取第三区域图表数据 ----设备运行模块
            ...
            if (isMock) {
                ...
                this.setDeviceRunTime(20000, reData); //数据回调处理 --两分钟
            } else {
                httpRequest('get', equipmentOperationApi, 'enterprise=' + this.enterpriseId, true, 1, 0, null, function(data) {
                    var reData = data.rows || [];
                    _this.setDeviceRunTime(20000, reData); //数据回调处理 --两分钟
                }, function(msg, code) {
                    alert(msg);
                });
            }
        ...
        getBarRChartData: function() { //获取第四区域图表数据 ----企业用电
            ...
            if (isMock) {
                ...
                httpRequest('get', enterpriseElectricityApi, 'enterprise=' + this.enterpriseId, true, 1, 0, null, function(data) {
                    var reData = data.object || {};
                    var reArr = reData.days || [];
        ...
        getChartSeries: function(dayList) {
            series = [];
            ...
            for (var i = 0; i < dayList.length; i++) {
                ...
                var items = this.getSeriesItemsForDay(i, dayItem);
        ...
        getSeriesItemsForDay: function(dayIndex, dayItem) {

            //var beforeSize = dayItem.dayList.length;

            //合并优化，保证运行时间不变，会移动时间合并。
            //dayItem.dayList  = this.mergeDuration(dayItem.dayList);


            var items = new Array();

            for (var i = 0; i < dayItem.stateDurations.length; i++) {

                var duration = dayItem.stateDurations[i];
                ...
                var SeriesItem = {
                    ...
                    type: 'bar',
                    stack: 'a',
                    data: [{ value: duration.duration, itemStyle: { color: color } }]

                };

                //补充其他天数的0
                for (var j = 0; j < dayIndex; j++) {
                    SeriesItem.data.unshift(0);
                }
        ...
        handleBarLChartData: function(daysArr) { //设备运行模块 -- 回调数据格式处理
            ...
            var seriesArr = this.getChartSeries(daysArr);
        ...
        handleLineChartData: function(reArr) { //生产状况 -- 回调数据格式处理
            ...
            for (var i = 0; i < reArr.length; i++) {
                ...
                for (var j = 0; j < itemArr.length; j++) {
                    if ($.inArray(itemArr[j].day, xAxisArr) < 0) {
                        xAxisArr.push(itemArr[j].day);
                    }
        ...
        handleMapChartOpt: function(chart) {
            ...
            for (var i = 0; i < areaLocations.length; i++) {
                ...
                marker.setAnimation(BMAP_ANIMATION_BOUNCE); //坠落动画
                points.push(point);
            }
        ...
        barLChartInit: function(id) { // 第三区域 ----设备运行模块
            ...
            var barLOption = option = {
                ...
                legend: {
                    icon: 'roundRect',
                    orient: 'vertical',
                    data: [{ name: '运行中', textStyle: { color: '#e69013' } }, { name: '未运行', textStyle: { color: '#fff' } }],
                    bottom: '8%',
                    right: '6%',
                    textStyle: {
                        color: '#fff',
